...
export default class MeiToolsAtomView {
  ...
  constructor() {
    ...
    // Create controls (currently just slider for zoom/scale)
    ...
    // container for on-the-fly styles (ex. change highlight color)
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      zoomCtrl.addEventListener('change', () => {
        options.scale = +zoomCtrl.value;
        this.reRenderNotation(item, vrvToolkit, verovioSVG, options);
      });

      // when highlight color is change, update css
  ...
  reRenderNotation(textEditor, toolkit, target, options = {}) {
    // re-render notation, optionally with new options
    if (options) {
      toolkit.setOptions(JSON.stringify(options));
    }
    target.innerHTML = toolkit.renderPage(1, {});
    this.addNotationEventListeners(target, textEditor);
    this.updateHighlight(textEditor);
  }
