...
export default class MeiToolsAtomView {
  ...
  constructor() {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      // wait for #verovioSVG, then set Verovio options and do initial render
      ...
      this.zoomCtrl.addEventListener('change', handleZoomChange = () => {
        this.updateAll(item)
      });

      // when pagination is enabled or disabled, update options and re-render notation
      this.layoutCtrl.addEventListener('change', handleLayoutToggle = () => {
      ...
      this.firstBtn.addEventListener('click', handleFirstClick = () => {
        this.updatePage(item, 'first');
      });
      this.prevBtn.addEventListener('click', handlePrevClick = () => {
        this.updatePage(item, 'prev');
      });
      this.nextBtn.addEventListener('click', handleNextClick = () => {
        this.updatePage(item, 'next');
      });
      this.lastBtn.addEventListener('click', handleLastClick = () => {
        this.updatePage(item, 'last');
      });

      // when highlight color is changed, add/update css in head of document
      this.hlghtCtrl.addEventListener('change', handleHighlightChange = () => {
  ...
  setVerovioOptions(newOptions = {}) {
    ...
    if (this.vrvOptions.noLayout === 0) {
      ...
      this.vrvOptions.pageWidth = Math.round(dimensions.pageWidth * (100 / this.vrvOptions.scale));
      this.vrvOptions.pageHeight = Math.round(dimensions.pageHeight * (100 / this.vrvOptions.scale));
    }
  ...
  changeCurrentPage(newPage) { // accepts number or string (first, last, next, prev)
    ...
    if (targetpage > 0 && targetpage <= this.pageCount) {
      console.log(`previous page: ${this.currentPage}, next page ${targetpage}, input: ${newPage}`);
